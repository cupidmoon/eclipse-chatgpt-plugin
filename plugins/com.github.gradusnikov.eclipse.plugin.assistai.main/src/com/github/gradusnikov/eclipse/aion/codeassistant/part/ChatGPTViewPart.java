...
import com.github.gradusnikov.eclipse.aion.codeassistant.part.Attachment.UiVisitor;
import com.github.gradusnikov.eclipse.aion.codeassistant.part.dnd.DropManager;
import com.github.gradusnikov.eclipse.aion.codeassistant.prompt.PromptParser;
...
public class ChatGPTViewPart
{
    ...
    public void createControls( Composite parent )
    {
        ...
        String[] comboItems = getModelList();
        Combo combo = new Combo(controls, SWT.DROP_DOWN);
        for(String comboItem: comboItems)
        {
        	combo.add(comboItem);
        }
        
        combo.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, buttons.length, 1));
    ...
    private String loadCss()
    {
        ...
        {
            try (InputStream in = FileLocator.toFileURL( new URL( "platform:/plugin/com.github.gradusnikov.eclipse.plugin.aion.codeassistant.main/css/" + file ) )
                    .openStream())
            {
    ...
    private String loadJavaScripts()
    {
        ...
        {
            try (InputStream in = FileLocator.toFileURL( new URL( "platform:/plugin/com.github.gradusnikov.eclipse.plugin.aion.codeassistant.main/js/" + file ) )
                    .openStream())
            {
        ...
        public void add( ImageData preview, String caption )
        {
            ...
            try
            {
                imageDescriptor = Optional.ofNullable( preview ).map( id -> ImageDescriptor.createFromImageDataProvider( zoom -> id ) ).orElse(
                        ImageDescriptor.createFromURL( new URL( "platform:/plugin/com.github.gradusnikov.eclipse.plugin.aion.codeassistant.main/icons/folder.png" ) ) );
            }
